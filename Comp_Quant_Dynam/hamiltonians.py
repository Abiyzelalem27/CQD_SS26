...
def double_well(x, lam):
    """function defines the potential energy V(x) of a quantum particle in a double-well system.
        for potential energy (V(x))= -1/2 * x^2 + λ * x^4 
    Parameters:
...
def time_evolution(H, psi0, t):
    """
    Compute quantum time evolution of a state using the Schrödinger equation.
    The evolution follows the unitary operator,  psi(t) = exp(-i H t) psi(0) 
    
    Parameters, H: Hamiltonian., psi0 : nitial state., t : time.

    Returns
    psi_t : Time-evolved quantum state.
    """

    # Time evolution operator (unitary)
    U = expm(-1j * H * t)
    # Apply operator to initial state
    return U @ psi0
